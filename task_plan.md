...
## 2026-03-06 Excel 数据集兼容与同场考试去重
- [x] 通读项目主链路（模型、解析、导入、分析、页面、测试）
- [x] 核对 `测试数据集/` 全部 6 个 Excel 文件的结构与现有覆盖
- [x] 识别“同一场考试的不同文件”风险并与用户确认口径
- [x] 写入设计文档与实施计划（同场考试等价去重）
- [x] 按 TDD 补充真实数据集回归测试
- [x] 实现考试等价标识提取与重复入库拦截
- [x] 校正导入预览/确认页的重复提示
- [x] 补充考试题型/板块语义的调研结论与可扩展点
- [x] 运行回归测试与 Django 自检
