...
## 2026-03-06 测试数据集兼容与同场考试识别

### 真实样本结构确认
- `2025-2026学年度第一学期期末考试【八年级】(英语)-八年级7班.xlsx`
  - `得分明细` + `班级英语成绩简表` + `班级英语成绩汇总`
  - 44 人，42 个题号，存在 2 条非数值总分与 1 条双 ID 不一致
- `2025-2026学年度第一学期期末考试【八年级】-英语-八7班-单科成绩单.xls`
  - 单 sheet、连续学生成绩单块
  - 42 人，42 个题号
- `期末模拟3.xlsx`
  - `得分明细` + `班级英语成绩简表` + `班级英语成绩汇总`
  - 44 人，72 个题号，双 ID 全量不一致（准考证号 vs 自定义考号）
...
### 已确认的业务风险
- 当前系统只按 `source_sha256` 去重，同一场考试的不同文件会被当成不同 `Exam`。
- 已确认的“同场异文件”样本：
  - `期末考试 xlsx` 与 `期末考试 xls`
  - `模拟1.xlsx` 与 `模拟1(1).xlsx`
- 风险外溢面：
  - `exam_list` 会出现重复考试
  - 学生总览 `exam_count` 会重复累加
  - 学生详情趋势线会重复出现同一场考试
  - 后续任务、分析、复盘都会建立在污染后的考试集合上

### 设计口径（用户已确认）
- 采用“阻止同一考试重复入库”的策略，不做分析阶段合并。
- 拦截时机：
  - 上传预览阶段尽早提示
  - 正式导入阶段再次兜底校验
- 同场考试判定优先依据：
  - 文件内部标题/工作表标题中的考试名、班级、满分、学段信息
  - 文件名兜底
  - 不依赖用户手输的 `exam_name`

### 题型/板块外部调研结论
- 已联网检索无锡地区初中英语期末/模拟卷公开资料，能找到的是“本地常见卷型结构”而不是当前仓库这 6 份 Excel 对应的精确原卷。
- 公开题型结构通常能大致落到：
  - 选择题 / 完形 / 阅读理解
  - 词汇或动词填空
  - 阅读表达
  - 书面表达
- 但当前数据集里同样出现了两种明显不同的题号密度：
  - `期末考试 xlsx/xls`：42 个题号
  - `期末模拟3.xlsx`：72 个题号（含更多子题）
- 结论：
  - 不能仅凭“无锡八年级英语”这个地区标签安全地把所有题号硬编码到统一题型区间
  - 更稳的做法应是给每场考试维护独立的题型元数据配置，再挂到 `identity_key`
  - 本轮不做高风险硬编码映射，只保留扩展位
