...
@pytest.mark.django_db
def test_toggle_submission_rejects_get(client) -> None:
    user_model = get_user_model()
    user_model.objects.create_user(username="t_get", password="p")
    assert client.login(username="t_get", password="p")
